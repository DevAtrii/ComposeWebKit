...
public class WebViewNavigator(private val coroutineScope: CoroutineScope) {
    private sealed interface NavigationEvent {
        ...
        data class ClearCache(val includeDiskFiles: Boolean) : NavigationEvent
    ...
    internal suspend fun WebView.handleNavigationEvents(): Nothing = withContext(Dispatchers.Main) {
        navigationEvents.collect { event ->
            when (event) {
                ...
                is NavigationEvent.ClearHistory -> clearHistory()
                is NavigationEvent.ClearCache -> clearCache(event.includeDiskFiles)
    ...
    public fun clearCache(includeDiskFiles: Boolean) {
        coroutineScope.launch { navigationEvents.emit(NavigationEvent.ClearCache(includeDiskFiles = includeDiskFiles)) }
    }
